...
const createUserInvitation = async (req, res) => {
  try {
    const result = await adminService.createUserInvitation(req.body);
...
const verifyAdminUserOtp = async (req, res) => {
  try {
    const result = await adminService.verifyAdminUserOtp(req.body);
...
const resendAdminUserOtp = async (req, res) => {
  try {
    const result = await adminService.resendAdminUserOtp(req.body);
    return res.status(200).json({
      success: true,
...
const updateUser = async (req, res) => {
  try {
    ...
    const updatedUser = await adminService.updateUser(req.params.id, req.body, currentAdminId);
    return res.status(200).json({
      success: true,
...
const updateUserStatus = async (req, res) => {
  try {
    const currentAdminId = getCurrentUserId(req);
    const { updatedUser, targetStatus } = await adminService.updateUserStatus(
      req.params.id,
      req.body.status,
      currentAdminId
    );
...
const deleteUser = async (req, res) => {
  try {
    const currentAdminId = getCurrentUserId(req);
    await adminService.deleteUser(req.params.id, currentAdminId);
    return res.status(200).json({
      success: true,
