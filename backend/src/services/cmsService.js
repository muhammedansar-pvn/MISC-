...
const getArticles = async (filter = {}) => Article.find(filter).sort({ createdAt: -1 });
const getArticleBySlug = async (slug) => Article.findOne({ slug });
...
const getDownloadResources = async (filter = {}) => DownloadResource.find(filter).sort({ publishedAt: -1 });
...
const getEnquiries = async (filter = {}) => Enquiry.find(filter).sort({ createdAt: -1 });
