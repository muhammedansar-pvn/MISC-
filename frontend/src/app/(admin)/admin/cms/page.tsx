...
import { FileText, Download, MessageSquare, Plus, RefreshCw, AlertCircle, Edit3, Trash2, X } from 'lucide-react';
...
import { Article, DownloadResource, Enquiry } from '@/types';

type CMSTab = 'articles' | 'resources' | 'enquiries';

export default function AdminCMSPage() {
  const [activeTab, setActiveTab] = useState<CMSTab>('articles');
  ...
  const fetchCMSData = async () => {
    ...
    try {
      const [artRes, resRes, enqRes] = await Promise.all([
        getArticles().catch(() => ({ success: false, data: [] })),
        getResources().catch(() => ({ success: false, data: [] })),
        getEnquiries().catch(() => ({ success: false, data: [] })),
      ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex space-x-2 border-b border-[#E2E8E0] pb-1 overflow-x-auto">
        {[
          { id: 'articles' as CMSTab, label: 'News & Articles', icon: FileText, count: articles.length },
          { id: 'resources' as CMSTab, label: 'Download Resources', icon: Download, count: resources.length },
          { id: 'enquiries' as CMSTab, label: 'Enquiries & Messages', icon: MessageSquare, count: enquiries.length },
        ].map((tab) => {
          const Icon = tab.icon;
          return (
            <button
      ...
      <div className="bg-white rounded-2xl border border-[#E2E8E0] shadow-xs overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center space-y-4">
            ...
            <button onClick={fetchCMSData} className="px-4 py-2 bg-[#2F7C7A] text-white font-bold text-xs rounded-lg uppercase cursor-pointer">
              ...
              <tbody className="divide-y divide-[#E2E8E0]">
                {articles.map((art) => (
                  <tr key={art._id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-xs text-slate-500">{art.createdAt ? new Date(art.createdAt).toLocaleDateString() : 'N/A'}</td>
                    <td className="px-6 py-4 text-right space-x-2">
                      <button onClick={() => handleOpenModal(art)} className="px-3 py-1.5 border rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 cursor-pointer">
                        <Edit3 className="w-3.5 h-3.5 inline mr-1" /> Edit
                      </button>
                      <button onClick={() => setDeleteConfirmId(art._id)} className="px-3 py-1.5 border border-rose-200 text-rose-600 rounded-lg text-xs font-semibold hover:bg-rose-50 cursor-pointer">
              ...
              <tbody className="divide-y divide-[#E2E8E0]">
                {resources.map((res) => (
                  <tr key={res._id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-right space-x-2">
                      <button onClick={() => handleOpenModal(res)} className="px-3 py-1.5 border rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 cursor-pointer">
                        <Edit3 className="w-3.5 h-3.5 inline mr-1" /> Edit
                      </button>
                      <button onClick={() => setDeleteConfirmId(res._id)} className="px-3 py-1.5 border border-rose-200 text-rose-600 rounded-lg text-xs font-semibold hover:bg-rose-50 cursor-pointer">
                        <Trash2 className="w-3.5 h-3.5 inline mr-1" /> Delete
                      </button>
                    </td>
                  </tr>
                  ...
                  <tr key={enq._id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-right">
                      <select
                        ...
                        className="px-2 py-1 text-xs border rounded-lg bg-white cursor-pointer"
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white max-w-sm w-full rounded-2xl p-6 text-center space-y-4">
            ...
            <div className="flex justify-center space-x-3 pt-2">
              <button onClick={() => setDeleteConfirmId(null)} className="px-4 py-2 border rounded-lg text-xs font-bold text-slate-700 cursor-pointer">Cancel</button>
              <button onClick={() => handleDelete(deleteConfirmId)} className="px-4 py-2 bg-rose-600 text-white rounded-lg text-xs font-bold uppercase cursor-pointer">Confirm Delete</button>
            </div>
          </div>
        </div>
      )}
              ...
              {activeTab === 'articles' ? (
                <>
                  ...
                  <div>
                    ...
                    <select value={formData.category || 'NEWS'} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="w-full px-3 py-2 border rounded-lg cursor-pointer">
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold uppercase mb-1">Category</label>
                      <select value={formData.category || 'SYLLABUS'} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="w-full px-3 py-2 border rounded-lg cursor-pointer">
                    ...
                    <div>
                      ...
                      <select value={formData.targetAudience || 'ALL'} onChange={(e) => setFormData({ ...formData, targetAudience: e.target.value })} className="w-full px-3 py-2 border rounded-lg cursor-pointer">
