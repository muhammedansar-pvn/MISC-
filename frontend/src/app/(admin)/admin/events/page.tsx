...
import { getEvents, createEvent, updateEvent, getEventRegistrations } from '@/services/event.service';
...
export default function AdminEventsPage() {
  ...
  const fetchEventsData = async () => {
    ...
    try {
      const [evtRes, regRes] = await Promise.all([
        getEvents().catch(() => ({ success: false, data: [] })),
        getEventRegistrations().catch(() => ({ success: false, data: [] })),
      ]);
  ...
  const handleOpenModal = (evt: EventModel | null = null) => {
    if (evt) {
      setEditingEvent(evt);
      setFormData({
        title: evt.title || '',
                ...
                {events.map((evt) => (
                  <tr key={evt._id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs text-slate-600">
                      {evt.startDate ? new Date(evt.startDate).toLocaleDateString() : 'N/A'}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => handleOpenModal(evt)} className="px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 cursor-pointer">
              ...
              <tbody className="divide-y divide-[#E2E8E0]">
                {registrations.map((reg) => (
                  <tr key={reg._id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4"><StatusBadge status={reg.status || 'CONFIRMED'} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-sm">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={formData.eventType || 'ACADEMIC'} onChange={(e) => setFormData({ ...formData, eventType: e.target.value })} className="w-full px-3 py-2 border rounded-lg cursor-pointer">
                ...
                <div>
                  ...
                  <input type="text" value={formData.venue || ''} onChange={(e) => setFormData({ ...formData, venue: e.target.value })} className="w-full px-3 py-2 border rounded-lg" />
                </div>
              </div>
              ...
              <div>
                ...
                <select value={formData.status || 'UPCOMING'} onChange={(e) => setFormData({ ...formData, status: e.target.value })} className="w-full px-3 py-2 border rounded-lg cursor-pointer">
