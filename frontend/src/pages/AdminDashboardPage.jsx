...
import { getDashboardStats, getUsers } from '../services/adminService';
  ...
  if (error) {
    return (
      <div className="bg-rose-50 border border-rose-200 rounded-xl p-8 text-center max-w-lg mx-auto space-y-4">
        ...
        <button
          onClick={fetchData}
          className="inline-flex items-center px-4 py-2 bg-[#2F7C7A] text-white font-bold text-xs rounded-lg uppercase tracking-wider hover:bg-[#256361] transition-all"
        >
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-[#E2E8E0] shadow-xs">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            className="inline-flex items-center justify-center px-4 py-2.5 border border-slate-300 text-slate-700 font-bold text-xs rounded-xl uppercase tracking-wider hover:bg-slate-50 transition-all"
          >
      ...
      <div className="bg-white rounded-2xl border border-[#E2E8E0] shadow-xs overflow-hidden">
        <div className="px-6 py-5 border-b border-[#E2E8E0] flex items-center justify-between bg-[#F7F8F5]">
          ...
          <button
            onClick={() => navigate('/admin/users')}
            className="text-xs font-bold text-[#2F7C7A] hover:underline flex items-center"
          >
              ...
              <tbody className="divide-y divide-[#E2E8E0]">
                {recentUsers.map((user) => (
                  <tr key={user._id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="px-6 py-4 font-semibold text-[#132238]">{user.name || 'N/A'}</td>
