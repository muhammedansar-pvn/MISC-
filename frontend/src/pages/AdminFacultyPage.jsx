...
import { UserCheck, Plus, Search, RefreshCw, AlertCircle, Edit3, X, User } from 'lucide-react';
...
export const AdminFacultyPage = () => {
  ...
  const handleOpenModal = async (fac = null) => {
    setFormError('');
    setFormLoading(true);
    setIsModalOpen(true);
    ...
    if (fac) {
      ...
      setSelectedUser(fac.userId || null);
      setFormData({
        userId: fac.userId?._id || fac.userId || '',
        facultyId: fac.facultyId || '',
        nameEnglish: fac.nameEnglish || fac.userId?.name || '',
        ...
        contactNumber: fac.contactNumber || fac.userId?.mobile || '',
  ...
  const filtered = facultyList.filter((f) => {
    ...
    const name = (f.userId?.name || f.nameEnglish || '').toLowerCase();
                ...
                {filtered.map((fac) => (
                  <tr key={fac._id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 font-bold text-[#132238]">
                      {fac.userId?.name || fac.nameEnglish || 'N/A'}
                      <p className="text-xs text-slate-400 font-normal">{fac.userId?.email || 'No email'}</p>
                    </td>
                    ...
                    <td className="px-6 py-4 text-xs text-slate-600">
                      ...
                      <p className="text-slate-400">{fac.institutionId?.name || 'Default Campus'}</p>
                    </td>
                    ...
                    <td className="px-6 py-4"><StatusBadge status={fac.userId?.status || 'ACTIVE'} /></td>
