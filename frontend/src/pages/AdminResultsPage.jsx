...
import { getExamResults, generateExamResults, getExams } from '../services/examService';
...
export const AdminResultsPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (exRes.data?.length > 0) setSelectedExamId(exRes.data[0]._id);
      if (clsRes.data?.length > 0) setSelectedClassId(clsRes.data[0]._id);
    } catch (err) {
      ...
      <div className="bg-white p-6 rounded-2xl border border-[#E2E8E0] shadow-xs space-y-4">
        ...
        <form onSubmit={handleGenerateResults} className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          ...
          <button
            type="submit"
            disabled={genLoading}
            className="w-full py-2.5 bg-[#2F7C7A] text-white font-bold text-xs rounded-xl uppercase tracking-wider hover:bg-[#256361] flex items-center justify-center"
          >
            {genLoading ? (
